...
from .config import (
    CONFIG, ADMIN_IDS, WHITELIST_FILE, BLACKLIST_FILE, STATS_FILE,
    BANWORDS_FILE, PERSONA_FILE, TOKEN_DAILY_FILE, SPLIT_STATE_FILE,
    PERSONA_CATALOG, HIDDEN_PERSONA_CATALOG, HIDDEN_PERSONA_WHITELISTS,
    reload_personas,
)
...
class PersonaManager:
    ...
    def _get_persona_item(self, persona_key: str) -> Dict[str, str]:
        if persona_key.isdigit():
            return PERSONA_CATALOG.get(int(persona_key), {})
        return HIDDEN_PERSONA_CATALOG.get(persona_key, {})
    ...
    def set_group_persona(self, group_id: str, persona_no: int) -> bool:
        if persona_no not in PERSONA_CATALOG: return False
        if not self._valid_persona(str(persona_no)): return False
    ...
    def resolve_persona_key(self, user_id: str, group_id: Optional[str] = None) -> str:
        ...
        if group_id:
            ...
            if gc is not None and gc in PERSONA_CATALOG and self._valid_persona(str(gc)):
                return str(gc)
        if self.global_persona in PERSONA_CATALOG and self._valid_persona(str(self.global_persona)):
    ...
    def resolve_system_prompt(self, user_id: str, group_id: Optional[str] = None) -> str:
        ...
        return sp or PERSONA_CATALOG.get(1, {}).get("system_prompt", "")
    ...
    def get_preview_text(self, user_id: str, extra_desc: str = "",
        ...
        for no in sorted(PERSONA_CATALOG.keys()):
            item = PERSONA_CATALOG[no]
...
class SessionManager:
    def __init__(self):
        ...
        self.split_enabled: Dict[str, bool] = {}
        ...
        if os.path.exists(SPLIT_STATE_FILE):
            try:
                with open(SPLIT_STATE_FILE, "r", encoding="utf-8") as f:
                    self.split_enabled = {str(k): bool(v) for k, v in json.load(f).items()}
    ...
    def set_split(self, user_id: str, enabled: bool):
        self.split_enabled[user_id] = enabled; self._save_split_state()
